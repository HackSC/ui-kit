import { Colors2023 } from "../styles/colors";
import React, {
  InputHTMLAttributes,
  useCallback,
  useEffect,
...
export interface OptionSelectProps
  ...
  options: ComboboxOption[] | (() => Promise<ComboboxOption[]>);
  onClickChooseOption?: (
...
export function Combobox(props: OptionSelectProps) {
  ...
  const [fuse, setFuse] = useState<Fuse<ComboboxOption> | null>(null);
  ...
  useEffect(() => {
    const f = new Fuse<ComboboxOption>([], { keys: ["displayName"] });
  ...
  const getOptionClickHandler = (option: ComboboxOption) => {
    return ((e) => {
